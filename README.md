...
개발자가 app-repo에 코드 Push

GitLab CI 파이프라인 실행

애플리케이션 이미지 빌드 및 사설 Registry Push

CI가 gitops-repo의 Kustomize 이미지 태그를 Commit SHA로 업데이트

Argo CD가 gitops-repo 변경사항 감지

Kubernetes에 자동 Sync 및 배포 반영

장애/드리프트 발생 시 Self-healing 수행
...
Phase 5. CI/CD
 ...
 kustomize edit set image로 gitops-repo 자동 업데이트
